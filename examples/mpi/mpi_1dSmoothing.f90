program smoothing1d
  ...
  integer :: iBlock,error,nBlock,RBlock,LBlock
  integer :: i,j,tag=1, RSrequest, RRrequest, LSrequest, LRrequest
  ...
  call MPI_Comm_size(MPI_COMM_WORLD, nBlock, error)
  
  ! Get the individual proc ID
  call MPI_Comm_rank(MPI_COMM_WORLD, iBlock, error)
  ...
  do i=1,nIter
     ...
     call MPI_Wait(RSrequest, status, error)
     ! wait for completion of receipt in ghost values
     call MPI_Wait(RRrequest, status, error)
     ! wait for completion of sending of boundary values
     call MPI_Wait(LSrequest, status, error)
     ! wait for completion of receipt in ghost values
     call MPI_Wait(LRrequest, status, error)
